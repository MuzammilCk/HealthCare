...
const CardNav = () => {
  ...
  const createTimeline = () => {
    ...
    gsap.set(navEl.querySelector('.card-nav-content-container'), { height: 0, opacity: 0 });
    gsap.set(cardsRef.current, { y: 50, opacity: 0 });

    const tl = gsap.timeline({ paused: true });

    tl.to(navEl.querySelector('.card-nav-content-container'), {
      height: 'auto',
      opacity: 1,
      duration: 0.4,
      ease: 'power3.out'
  ...
  return (
    <nav ref={navRef} className={`card-nav ${isExpanded ? 'open' : ''} mx-4 lg:mx-6 mt-6`}>
