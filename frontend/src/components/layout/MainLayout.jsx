...
function NavLink({ to, label, onClick }) {
  ...
  return (
    <Link to={to} onClick={onClick} className={`block px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-200 ${active ? 'bg-primary text-white' : 'text-text-secondary hover:bg-primary/10 hover:text-text-primary'}`}>{label}</Link>
  );
}
...
export default function MainLayout({ role }) {
  ...
  return (
    <div className="min-h-screen flex">
      {/* --- Desktop Sidebar --- */}
      <aside className="hidden md:flex md:flex-col w-64 bg-bg-card/80 backdrop-blur-xl border-r border-white/20 p-4">
        <div>
          <div className="text-2xl font-bold text-primary-dark mb-6">HealthSync</div>
