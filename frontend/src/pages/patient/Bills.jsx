...
import { DollarSign, FileText, Calendar, User, CheckCircle, Clock, X } from 'lucide-react';
...
export default function PatientBills() {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'paid':
        return 'bg-success/15 text-success-fg border-success/20';
      case 'unpaid':
        return 'bg-amber-400/15 text-amber-500 border-amber-500/20';
      case 'cancelled':
  ...
  const BillDetailsModal = ({ bill, onClose }) => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div className="glass rounded-xl shadow-card max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-border">
          <div className="sticky top-0 bg-background/95 backdrop-blur border-b border-border p-6 flex items-center justify-between">
            <h2 className="text-2xl font-bold text-foreground">Bill Details</h2>
            <button
              onClick={onClose}
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-foreground/5 rounded-lg p-4 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-mono text-foreground">{bill._id.slice(-8)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Doctor:</span>
                <span className="font-semibold text-foreground">Dr. {bill.doctorId?.name}</span>
              </div>
              <div className="flex justify-between text-sm">
            ...
            {/* Bill Items */}
            <div>
              <h3 className="font-semibold text-foreground mb-3">Bill Items</h3>
              <div className="space-y-2">
                {bill.items.map((item, index) => (
                  <div key={index} className="flex justify-between items-center p-3 bg-foreground/5 rounded-lg">
                    <div className="flex-1">
                      <p className="font-medium text-foreground">{formatDescription(item.description)}</p>
                      <p className="text-sm text-muted-foreground">Quantity: {item.quantity}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold text-foreground">{formatAmount(item.amount * item.quantity)}</p>
                      <p className="text-xs text-muted-foreground">{formatAmount(item.amount)} each</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {bill.notes && (
              <div>
                ...
                <p className="text-sm text-muted-foreground bg-foreground/5 p-3 rounded-lg">{bill.notes}</p>
              </div>
            )}

            {/* Total */}
            <div className="bg-brand-cyan/10 rounded-lg p-4 border-2 border-brand-cyan/20">
              <div className="flex justify-between items-center">
                <span className="text-lg font-semibold text-foreground">Total Amount</span>
                <span className="text-2xl font-bold text-brand-cyan-fg">{formatAmount(bill.totalAmount)}</span>
              </div>
            </div>
            ...
            {bill.status === 'paid' && bill.paidAt && (
              <div className="bg-success/10 border border-success/20 rounded-lg p-4">
                <div className="flex items-center gap-2 text-success-fg mb-2">
  ...
  return (
    <div className="space-y-6 bg-background text-foreground">
      ...
      {/* Main Tabs */}
      <div className="glass rounded-xl shadow-card p-2 flex gap-2 mb-4">
        {[
          { value: 'bills', label: 'My Bills', icon: <FileText className="w-4 h-4" /> },
          { value: 'payments', label: 'Payment History', icon: <DollarSign className="w-4 h-4" /> }
        ].map((tab) => (
          <button
            key={tab.value}
      ...
      {activeTab === 'bills' && (
        <div className="glass rounded-xl shadow-card p-2 flex gap-2">
          {[
      ...
      {activeTab === 'bills' ? (
        ...
        <div className="glass rounded-xl shadow-card p-12 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {bills.map((bill) => (
            <div
              key={bill._id}
              className="glass rounded-xl shadow-card p-6 hover:shadow-glow transition-shadow cursor-pointer"
              onClick={() => setSelectedBill(bill)}
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-brand-cyan/15 rounded-full flex items-center justify-center">
                    <FileText className="w-6 h-6 text-brand-cyan-fg" />
                  </div>
                  <div>
                    <h3 className="font-semibold text-foreground">Bill #{bill._id.slice(-6)}</h3>
                    <p className="text-sm text-muted-foreground">
              ...
              <div className="border-t border-border pt-4 flex items-center justify-between">
                ...
                <span className="text-xl font-bold text-brand-cyan-fg">{formatAmount(bill.totalAmount)}</span>
              </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-border">
                  {payments.map((payment) => (
                    <tr key={payment._id} className="hover:bg-foreground/5">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                          payment.paymentType === 'booking_fee' ? 'bg-brand-sky/15 text-brand-sky-fg' :
                          payment.paymentType === 'bill_payment' ? 'bg-brand-violet/15 text-brand-violet' :
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold">
                        <span className={payment.paymentType === 'refund' ? 'text-success-fg' : 'text-foreground'}>
                          {payment.paymentType === 'refund' ? '+' : ''}{formatAmount(payment.amount)}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                          payment.status === 'completed' ? 'bg-success/15 text-success-fg' :
                          payment.status === 'pending' ? 'bg-amber-400/15 text-amber-500' :
                          'bg-error/15 text-error-fg'
                        }`}>
                          {payment.status}
                        </span>
                      </td>
